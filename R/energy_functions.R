...
#'   scaled by metabolic body mass. The returned value carries \code{unit},
#'   \code{source}, and \code{full_name} attributes.
#'
#' @param bm Numeric. Body mass (kg).
#'
...
#'
#' @description Returns the daily heat increment of feeding: the Chappel and
#'   Hudson (1978b) per-body-mass hourly rate (\code{HIF}) times body mass, times
#'   the proportion of the diurnal period spent feeding (\code{prop_day_forage};
#'   Courtemanch et al. 2014), times the day length in hours. Day length is read
#'   from the precomputed per-time-step \code{day_length} vector built by
#'   \code{.refresh_daylight()} from study latitude and day of year.
#'
#' @param bm Numeric. Body mass (kg).
...
#'   day's steps. Each step's cost is the Dailey and Hobbs (1989) distance cost
#'   factor for the step's slope class, times body mass, times horizontal step
#'   length. Slope is computed from DEM elevations at the step's start and end
#'   coordinates. The caller supplies the day's coordinate and step length windows
#'   and the body mass.
#'
#' @details Elevations are looked up by cell number in \code{dem_vals}, and the cell
#'   numbers come from \code{terra::cellFromXY()} on the \code{dem} raster, which
#'   this function reads from the global environment, where \code{ncc_abm()} places
#'   it. All model rasters share one grid and coordinate reference system, so the
#'   step coordinates need no reprojection.
#'
#' @param x_window Numeric vector of x coordinates for the rows spanning the day's
#'   steps (the step rows plus the row immediately preceding the first).
#' @param y_window Numeric vector of y coordinates, aligned with \code{x_window}.
#' @param sl_window Numeric vector of the day's step lengths.
...
calc_energy_loc <- function(x_window, y_window, sl_window, bm, dem_vals) {
  ...
  #1) step start and end coordinates: with n steps, the starts are rows 1 to n of the window and the ends are rows
  #   2 to n + 1
  ...
  #2) start and end elevations, looked up by cell number in the precomputed dem values

  elev_start <- dem_vals[terra::cellFromXY(dem, start)]
  elev_end <- dem_vals[terra::cellFromXY(dem, end)]

  #3) the signed slope of each step in degrees: negative for descent, positive for incline

  slope <- atan2(elev_end - elev_start, sl_window) * 180 / pi

  #4) the distance cost factor of each step, selected by slope class: descent steeper than 10 degrees, descent of
  #   1 to 10 degrees, flat (within 1 degree), incline of 1 to 10 degrees, and incline steeper than 10 degrees. a
  #   step with an NA slope keeps an NA factor, so the day's locomotion energy is NA
  ...
  #5) the daily cost: factor (J per kg per m) times body mass times horizontal step length, summed over the steps
  #   and converted from joules to kilojoules
...
#'   scaled by the lactation modifier at the agent's days post partum. Returns 0 for
...
#'   lactation) to give net energy balance.
#'
#' @param energy_i_window Numeric vector of the day's hourly energy intakes (kJ).
...
calc_energy_net <- function(energy_i_window, energy_bmr, energy_hif, energy_loc, energy_rep) {
  ...
  #2) net balance: intake minus basal metabolism, heat increment of feeding, locomotion, and lactation

  net <- daily_intake - energy_bmr - energy_hif - energy_loc - energy_rep

  #3) return both. unname() strips any names carried in from the inputs so the two elements are named exactly
  #   daily_intake and energy_net
...
calc_fat_change <- function(net) {
  ...
  #1) the change in fat mass in grams: net energy (kJ) divided by the energy density of fat (kJ/g). the sign of
  #   the balance carries through, so a surplus gives a gain and a deficit a loss
